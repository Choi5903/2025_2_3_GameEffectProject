...
public class ChainShoot : MonoBehaviour
{
    ...
    IEnumerator UpdateLineRenderer(GameObject lineR, Transform startPos, Transform endPos, bool getClosestEnemyToPlayer = false)
    {
        if (shooting && shot && lineR != null)
        {
            ...
            {
                StartCoroutine(UpdateLineRenderer(lineR, startPos, playerEnemyDetector.GetClosedEnemy().transform, true));

                if (currentClosestEnemy != playerEnemyDetector.GetClosedEnemy())
                {
                    StopShooting();
                    StartShooting();
                }
    ...
    IEnumerator ChainReaction(GameObject closestEnemy)
    {
        ...
        else
        {
            if (shooting)
            {
                ...
                if(!enemiesInChain.Contains(closestEnemy.GetComponent<EnemyDetector>().GetClosedEnemy()))
                {
                    NewLineRenderer(closestEnemy.transform, closestEnemy.GetComponent<EnemyDetector>().GetClosedEnemy().transform);
                    StartCoroutine(ChainReaction(closestEnemy.GetComponent<EnemyDetector>().GetClosedEnemy()));
                }
            }
        }
    }
    ...
    void StartShooting()
    {
        ...
        if (playerEnemyDetector != null && playerFirePoint != null && lineRendererPrefab != null)
        {
            if (!shot)
            {
                ...
                currentClosestEnemy = playerEnemyDetector.GetClosedEnemy();
                NewLineRenderer(playerFirePoint, playerEnemyDetector.GetClosedEnemy().transform, true);

                if (maximunEnemiesInChain > 1)
                {
                    StartCoroutine(ChainReaction(playerEnemyDetector.GetClosedEnemy()));
                }
            }
        }
    }
